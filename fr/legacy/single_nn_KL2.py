...
def t_distribution_torch(x1, x2, axis=1):
	return 1 / (1 + (x1 - x2).pow(2).sum(axis=axis))


def gaussian_torch(x1, x2, sigma=1, axis=1):
	return torch.exp(-(x1 - x2).pow(2).sum(axis=axis) / (2 * sigma ** 2))
	# return 1/(x1 - x2).pow(2).sum(axis=axis)
	# return t_distribution_torch(x1, x2, axis=axis)
...
def compute_j_i(X, i, sigma_i=1):
	...
	tmp = gaussian_torch(X, X[i], sigma_i, axis=1).detach().numpy()
	tmp[i] = 0  # exclude the ith item
	sum_x_ji_ = np.sum(tmp)
...
def compute_y_kl(X, net):
	...
	for k in range(N):
		...
		for l in range(N):
			if l == k: continue
			y_l = net(X[l])
			sum_y_kl += t_distribution_torch(y_k, y_l, axis=0).detach().numpy().item()
	return sum_y_kl
...
def compute_entropy(X, i, sigma):
	...
	for j in range(N):
		if i == j: continue
		p_ji_ = gaussian_torch(X[i], X[j], sigma, axis=0).detach()/compute_j_i(X, i, sigma_i=sigma)
		if p_ji_ == 0:
			H += 0
		else:
...
def compute_P_ij(X, perplexity=5):
	...
	for i in range(N):
		...
		sum_x_ji_ = np.clip(sum_x_ji_, 1e-20, 1e+20)
		max_sum_x_ji_ = max(max_sum_x_ji_, sum_x_ji_)
		for j in range(N):
			if i == j: continue
			X_j = X[j]
			p_ji_ = gaussian_torch(X_i, X_j, sigma_i, axis=0).detach().numpy().item() / sum_x_ji_   # fixed X_i. X_i picks X_j as its neighbour.
...
def main():
		...
		for epoch in range(n_epochs + 1):
			...
			for i in range(N):  # O(N**2)
				...
				# 	X_j = X[j]
					...
					y_j = net(X_j)
					q_ij = t_distribution_torch(y_i, y_j, axis=0) / sum_y_ij
					...
					# if q_ij.detach().numpy().item() < 0:
					# 	print(f'{i}, {j}, q_ij: {q_ij}')
					# if q_ij == 0:
				...
				ed = time.time()
				# if i%10==0: print(f'i: {i} takes {ed-st}s')
			loss.backward()
			...
			L += loss.item()
			ed = time.time()
			print(f'{epoch + 1}/{n_epochs} takes {ed - st}s, loss: {L}, s1: {s1}, s2:{s2}, base: {base}')
	...
	for i in range(N):
		...
		for j in range(i + 1, N):
			...
			y_j = net(X[j])
			q_ij = t_distribution_torch(y_i, y_j, axis=0).detach().numpy().item() / sum_y_ij
