...
def gaussian_torch(xi, u_i, sigma2_i=1):
	# return torch.exp(-(x1 - x2).pow(2).sum() / (2 * sigma ** 2))
	return torch.exp(-(xi-u_i) @ torch.inverse(sigma2_i) @ (xi-u_i).T/2)
...
def compute_sigma_i(X2, u_i, verbose=1):
	"""

	"""
	N,d = X2.shape
	sigma_i = (X2-u_i) * (X2-u_i).T / N
...
def compute_P_ij(X, perplexity=5, knn=None):
	...
	for i in range(N):
		X_i = X[i]
		u_i = X_i
		indices = knn.kneighbors(X_i.detach().numpy().reshape(1, -1), n_neighbors=perplexity, return_distance=False)[0]
		# indices = knn.radius_neighbors(X[i].detach().numpy().reshape(1, -1), radius=10.0, return_distance=False)[0]
		X2 = X[indices]
		sigma2_i = (torch.matmul((X2-u_i).T, (X2-u_i))/ N)     # matrix,
		length = X2.shape[1]
		sigma2_i[range(length), range(length)] = torch.diag(sigma2_i) + 1e-8
		# sigma_i = 1  # use fixed sigma
		# precompute the sum/denominator for each X_i with the best sigma_i
		sum_x_ji_ = torch.clamp(torch.exp(-(X2-u_i) @ torch.inverse(sigma2_i) @ (X2-u_i).T/2).sum(), min=1+1e-8, max=1e+8)
		sum_x_ji_ = sum_x_ji_ - 1 #  excluding X_i (np.exp(0) == 1 ) from the sum
		max_sum_x_ji_ = max(max_sum_x_ji_, sum_x_ji_)
		print(i, max_sum_x_ji_)
		for j in range(N):
			if i == j: continue
			X_j = X[j]
			p_ji_ = gaussian_torch(X_j.reshape(1, -1), u_i.reshape(1, -1), sigma2_i).detach().numpy().item() / sum_x_ji_   # fixed X_i. X_i picks X_j as its neighbour.
			P_ij[tuple(zip(X[i].detach().numpy(), X[j].detach().numpy()))] = (p_ji_, sigma2_i, sum_x_ji_)
...
def main():
	...
	X_raw, y_raw = gen_data.gen_data(n=5, data_type=data_name, is_show=False, random_state=2)
	...
	plt.scatter(X_[:, 0], X_[:, 1], c=y)
	plt.title(f'PCA X.shape: {X.shape}')
	plt.show()
	...
	perplexity = 10
	base = 10
	step = base / ((n_epochs - 1) / 10)  # the last 5 epochs, use p_ij
	...
	if not os.path.exists(model_file):
		...
		sigmas = {}
		sum_x = {}
		P_ij = compute_P_ij(torch.from_numpy(X_raw).float(), perplexity, knn)
			...
			for i in range(N):  # O(N**2)
				...
				for j in range(0, N):
					...
					# 	w_ij = 100 * p_ij
						...
						# w_ij = 1
						w_ij = p_ij
					w_ij = torch.clamp(w_ij, min=1, max=1e+8)
					# w_ij = base ** p_ij if base > 1 else p_ij
					# w_ij = p_ij**(1/2)
					# w_ij = 1/p_ij**2
					# loss += w_ij * torch.log2(p_ij / q_ij)  # if p_ij < q_ij: log2 < 0; otherwise, log2 >=0
					p_ij = np.clip(p_ij, 1e-10, 1e+8)
					q_ij = torch.clamp(q_ij, min=1e-10, max=1e+8)
					loss += w_ij * torch.log2(p_ij / q_ij).abs()
					# print(w_ij, p_ij, q_ij, torch.log2(p_ij / q_ij).abs())
					# if q_ij == 0:
