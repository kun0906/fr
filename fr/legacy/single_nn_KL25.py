...
def compute_P_ij(X, perplexity=5):
	...
	for i in range(N):
		...
		# # find the best sigma for each X_i and given perplexity using entropy: perplexity = 2 ** H(Pi)
		# sigma_i = compute_sigma_i(X, i, perplexity) # O(r*N**2)
		sigma_i = (1/2)**(1/2)  # use fixed sigma
...
def main():
	...
	X_raw, y_raw = gen_data.gen_data(n=100, data_type=data_name, is_show=False, random_state=2)
	...
	if not os.path.exists(model_file):
		...
		sigmas = {}
		sum_x = {}
		P_ij = compute_P_ij(torch.from_numpy(X_raw).float(), n_neighbors)
		...
		for epoch in range(n_epochs + 1):
			...
			for i in range(N):  # O(N**2)
				...
				p = 1
				q = 1
				sum_y_ji_  = Q_ij[tuple(X[i].detach().numpy())]
