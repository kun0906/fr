...
def main():
	...
	if not os.path.exists(model_file):
		...
		for epoch in range(n_epochs + 1):
			...
			Y = net(X)
			Q, max_q = compute_P(Y.detach().numpy())
			for i in range(N):  # O(N**2)
				for j in range(N):
					if i == j: continue
					# p, _, _ = P[tuple(zip(X[i].detach().numpy(), X[j].detach().numpy(), X[k].detach().numpy()))]
					p = dist2_tensor(X[i], X[j]) / max_p  # normalize to [0, 1]
					q = dist2_tensor(Y[i], Y[j]) / max_q  # normalize to [0, 1]
					...
					# loss += w * torch.log2(p / q).abs()  #+ dist2_tensor(cos_sim_torch(X[i], X[j]), cos_sim_torch(Y[i], Y[j]))
					alpha = 3
					loss += w * (p - alpha * q).pow(2)
					# # loss += w * (1-p/q).pow(2)
					# loss1 = 0
					# for k in range(0):
					# 	if k == j or k == i: continue
					# 	loss1 += 1 * dist2_tensor(cos_sim_torch(X[i]-X[j], X[i] - X[k]), cos_sim_torch(Y[i]-Y[j], Y[i] - Y[k]))
					# 	loss1 += 1 * dist2_tensor(cos_sim_torch(X[i]-X[j], X[j] - X[k]), cos_sim_torch(Y[i]-Y[j], Y[j] - Y[k]))
					#
					# 	# p = dist2_tensor(X[i], X[k]) / max_p  # normalize to [0, 1]
					# 	# q = dist2_tensor(Y[i], Y[k]) / max_q  # normalize to [0, 1]
					# 	# w = torch.exp(-p)
					# 	# loss += w * torch.log2(p / q).abs()  # + dist2_tensor(cos_sim_torch(X[i], X[j]), cos_sim_torch(Y[i], Y[j]))
					# 	# print(cos_sim_torch(Y[i]-Y[j], Y[i] - Y[k]), Y[i]-Y[j], Y[i] - Y[k])
					ds.append((p, q.detach().numpy().item()))
					# loss += loss1 / (2*N)
