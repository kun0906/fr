...
def _show_update_data(tsne, inc_tsne, out_dir, idx=0):
	...
	ax[0].text(n1 + 0.1, position, f'tsne (n1:{n1})', rotation=90)  # (x, y, text)
	# acc_1nn
	...
	ax[1].text(n1 + 0.1, position, f'tsne (n1:{n1})', rotation=90)  # (x, y, text)
	...
	ax[0].axvline(x=n1, ls='-', color='black')
	position = float(fmt((max(trust) + min(trust)) / 2))
	ax[0].text(n1 + 0.1, position, f'inc_tsne n1:{n1}', rotation=90)  # (x, y, text)
	...
	ax[1].axvline(x=n1, ls='-', color='black')
	position = float(fmt((max(acc) + min(acc)) / 2))
	ax[1].text(n1 + 0.1, position, f'inc_tsne n1:{n1}', rotation=90)  # (x, y, text)
