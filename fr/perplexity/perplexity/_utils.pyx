...
cpdef _binary_search_perplexity(
    ...
    Beta : array, shape (n_samples, )
        Beta[i] = 1/(2*sigma**2) of each sample 
    """
    ...
    cdef cnp.ndarray[cnp.float64_t, ndim=1] S_Pi = np.zeros(
        (n_samples,), dtype=np.float64)

    cdef cnp.ndarray[cnp.float64_t, ndim=1] Beta = np.zeros(
        (n_samples, ), dtype=np.float64)
...
# cpdef cnp.ndarray[cnp.float32_t, ndim=2] _update_binary_search_perplexity(
#         cnp.ndarray[cnp.float32_t, ndim=2] sqdistances,
#         cnp.ndarray[cnp.float32_t, ndim=1] Beta,
#         int verbose):
#     """Binary search for sigmas of conditional Gaussians.
#
#     This approximation reduces the computational complexity from O(N^2) to
